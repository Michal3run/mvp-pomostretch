...
- **Root Cause**: The `rule-engine.ts` is mathematically designed to randomize _only if_ there are more than 3 matching fresh candidates. Since the M1 seed catalog only contains 2 exercises for "Tylko oczy", all 2 are selected and then predictably sorted by duration (shortest first).
...
- **Observation**: After finishing exercises, the app asks to start a new 25m work session. The user expected a 5 or 10-minute break timer countdown _after_ or _instead of_ the exercises.
