...
**Why this happened**: When LLMs generate implementation plans, their attention mechanism focuses on the _feature intent_ (e.g., "show a warning") and often drops the _strict architectural constraints_ (e.g., "Astro components cannot run in React"). The LLM "knows" the rule, but fails to apply it unless forced to verify boundaries explicitly.
...
### L13: Astro Cloudflare Adapter Silently Overrides Wrangler Config — `--env` Is Broken

**Date**: 2026-08-01  
**Context**: M2 deployment, attempted `npx wrangler deploy --env dev` to deploy to a safe preview worker.

**Problem**: Ran `npx wrangler deploy --env dev` expecting deployment to `pomo-stretch-dev` worker. Instead, wrangler deployed to **production** (`pomo-stretch`). No error, no warning — silent production deploy.

**Root cause**: Astro's `@astrojs/cloudflare` adapter generates its own `dist/server/wrangler.json` during `astro build`. When you run `npx wrangler deploy`, wrangler detects this file and uses it **instead of** the user's `wrangler.jsonc`. The generated file has `"name": "pomo-stretch"` hardcoded and does NOT inherit `env` blocks from the original config. The `--env dev` flag is silently ignored because the active config has no env definitions.
...
**Why this is dangerous**: An AI agent reading `wrangler.jsonc` sees `env.dev.name = "pomo-stretch-dev"` and reasonably concludes `--env dev` will deploy there. The command runs without errors, looks successful, and silently pushes to production. There is zero indication in stdout that the env flag was ignored.

**Solution**:

- Use `--name pomo-stretch-dev` flag instead of `--env dev`. The `--name` flag overrides the worker name regardless of which config file wrangler uses.
- For secrets on the dev worker, also use `--name`: `npx wrangler secret put SUPABASE_URL --name pomo-stretch-dev`

**Guideline**: In any project using Astro + Cloudflare Workers, **never use `wrangler deploy --env`**. Always use `wrangler deploy --name <worker-name>` to explicitly select the deployment target. Document the exact deploy commands in `AGENTS.md` so agents copy-paste the safe version instead of reasoning from first principles and getting it wrong.

---

### L14: Keyword Matcher Domain Enums Must Align with DB Schema, and Graceful Fallback Enforcement

**Date**: 2026-08-01  
**Context**: M3 Break Input implementation review (`src/pages/api/break-input.ts`).

**Problem**:

1. **Tag Mismatch**: The implementation mapped UI input to localized Polish tags (`"kark"`, `"oczy"`, `"ogólne"`). However, the database schema (`supabase/migrations/20260715120000_create_exercise_table.sql`) seeds `body_areas` with English identifiers (`'eyes'`, `'neck'`, `'shoulders'`, `'lower_back'`, `'general'`). As a result, downstream M4 rule engine queries would return 0 exercises.
2. **Missing Fallback (FR-012)**: Unrecognized free-text inputs produced `tags: []` rather than falling back to `["general"]`, violating the PRD guarantee that users always receive an exercise sequence.
3. **Hardcoded Secure Cookie Flag**: `secure: true` in cookie options caused cookies to be rejected in unencrypted local HTTP dev environments (`http://localhost:4321`).

**Solutions applied**:

- Map all user inputs (quick-picks and Polish/English free-text keywords) directly to the canonical DB `body_areas` keys (`eyes`, `neck`, `shoulders`, `lower_back`, `general`).
- Enforce `if (tags.length === 0) tags = ["general"];` to satisfy FR-012.
- Set `secure: import.meta.env.PROD` so cookies work both in local HTTP dev and HTTPS production.
