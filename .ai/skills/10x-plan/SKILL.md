...
9. **Strict Contract Verification (per Lesson L11)**:
   - Before generating the final plan, you MUST explicitly cross-check your proposed phases against `AGENTS.md` and `lessons.md`.
   - **Boundary Check**: If your plan mounts a React island (`client:load`), explicitly verify you are not attempting to render an Astro component inside it.
   - **Convention Check**: Verify that shared types are placed in `src/types.ts` as dictated by project rules.
   - **Contract Language**: Never write "if required", "as needed", or "refactor accordingly". Make the architectural decision hard and explicit.
